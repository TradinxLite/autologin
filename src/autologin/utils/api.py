...
def get_auto_code_fyers(fy_id, app_id, totp_key, pin, redirect_uri, app_type):
    send_otp_result = send_login_otp(fy_id=fy_id, app_id=app_id)
...
def generate_dhan_consent():
    ...
    url = "https://api.cirrus.trade/broker-accounts/dhan/generate-consent/"
    try:
        response = requests.get(url=url)
        response.raise_for_status()
        response_data = response.json()
        if not response_data.get('consent_id'):
            return None
        return response_data["consent_id"]
    except Exception as e:
        print("Exception in generate_dhan_consent", str(e))
        return None
